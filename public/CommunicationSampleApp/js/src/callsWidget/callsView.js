(function (window, $) {
    ...
    function CallsView(handlers) {
        ...
        this.callOTT = $('#callOTT');

        this.makeAudioCallBtn.off('click').on('click', function () {
            var remoteAddress = this.calleeAddress.val();
            handlers.handleAudioMakeCall(remoteAddress, this.callOTT.is(":checked"));
        }.bind(this));

        this.makeVideoCallBtn.off('click').on('click', function () {
            var remoteAddress = this.calleeAddress.val();
            handlers.handleVideoMakeCall(remoteAddress, this.callOTT.is(":checked"));
        }.bind(this));
