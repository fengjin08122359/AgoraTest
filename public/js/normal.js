(function($) {
    ...
    $(function() {
        ...
        var resolution = param["resolution"]||Cookies.get("resolution") || "720p",
            maxFrameRate = Number(param["maxFrameRate"]||Cookies.get("maxFrameRate") || 30),
            //maxBitRate        = Number(Cookies.get("maxBitRate") || 750),
            channel = param["roomName"]||Cookies.get("roomName"),
            remoteStreamList = [],
        ...
        //$("#room-name-meeting").html(visitorName);
		...
		function displayNclient(){
			
			if (remoteStreamList.length === 0) {
                clearAllStream();
                size = calculateVideoSize(false);
				
                displayStream("agora-local", localStream, $("#video-container").width(), $("#video-container").height(), '');
                ...
                displayStream("agora-remote", remoteStreamList[0].stream, $("#video-container").width(), $("#video-container").height(), '');
		...
		function displayNclientDiv(tagId, stream, width, height, className, parentNodeId) {
        ...
        function displayStream(tagId, stream, width, height, className, parentNodeId) {
            ...
            if (className.indexOf('local-partner-video') > -1) {
                var videoWidth = $().height()
                var left = (1200 - videoWidth) / 2 + 12;

                styleStr += 'top:12px; left:' + 12 + 'px;';
            }
